...
### Eje de decisión — approach de GENERACIÓN de imágenes (RESUELTO: opción 1, [ADR-011](../adr/011-satellite-image-assets.md))
> **APROBADO por el operador (gate humano, 2026-06-22): opción 1** (hosted Flux vía gateway multi-proveedor, **pluggable** a Firefly/self-hosted). Las opciones investigadas que sustentan la decisión:
...
**Decisión APROBADA (gate humano, 2026-06-22): Opción 1** (Flux hosted vía un **gateway multi-proveedor**,
horneado a `public/`) como **default**, por la relación calidad/coste/ownership y porque el proveedor queda
**pluggable**: permite escalar a **(2) Firefly** cuando un cliente exija indemnización de copyright, o a **(3)
self-hosted** si el volumen lo justifica. **Siempre** se prefiere el **asset real** para lo documental; la
generación cubre lo **decorativo / lo que falta**. **Optimización de imágenes pobres:** normalizar/redimensionar
al hornear (`next/image` + formatos modernos) — el detalle, en el ECO de F7 ([ECO-61](https://emillionnetworking-ltd-labs.atlassian.net/browse/ECO-61)).
Registrada en [ADR-011](../adr/011-satellite-image-assets.md).
...
**Pilar de imágenes/assets (re-aim ECO-60 — APROBADO; [ADR-011](../adr/011-satellite-image-assets.md); ECO de seguimiento [ECO-61](https://emillionnetworking-ltd-labs.atlassian.net/browse/ECO-61)).**
- **Fase 7 — IMÁGENES / ASSETS (P5):** ingerir los assets REALES del cliente a `public/` (logo siempre on-screen + fotos reales); **generación IA de un set decorativo original y coherente** por cliente (**hosted Flux vía gateway multi-proveedor, pluggable** — opción 1 aprobada, ver §«Eje de decisión»); línea decorativo-vs-real (logo/real nunca generados; generado = decorativo, jamás un hecho falso); optimización de imágenes pobres (`next/image` + formatos modernos); firma *"Powered by EM Ecosystem"* en el footer. *(depende de F4/F5; cross-mode a/b/c/d/e; [ADR-011](../adr/011-satellite-image-assets.md))*
