    ...
    <div className="flex items-center gap-0.5" aria-label={`${rating} de 5 estrellas`}>
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          size={size}
          className={n <= rating ? "fill-accent text-accent" : "fill-border-strong text-border-strong"}
        />
      ))}
    </div>
  );
    ...
    <div ref={ref} className={`card-flat ${className}`} style={style}>
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className="text-body font-semibold text-content-primary">{review.name}</p>
          <p className="text-caption text-content-tertiary">{review.relativeDate}</p>
        </div>
      </div>
      <div className="mt-3">
        <StarRating rating={review.rating} />
      </div>
      <p className="mt-3 text-body leading-relaxed text-content-secondary">{review.text}</p>
    </div>
  );
}
        ...
        <section className="bg-surface-primary py-20">
          <div className="mx-auto max-w-7xl px-6">
            <div className="mb-10 text-center">
              <p className="text-[18px] leading-7 md:text-h1 font-semibold tracking-wide text-accent">Testimonios »»</p>
              <h1 className="mt-2 text-display text-content-primary">Lo dicen ellos.</h1>
              <p className="mx-auto mt-3 max-w-xl text-base text-content-secondary">
                Reseñas verificadas en Google Maps de personas que han trabajado con Cristian.
              </p>
            </div>

            {/* Aggregate rating + mock disclaimer */}
            <div className="mx-auto mb-10 flex max-w-md flex-col items-center gap-3 text-center">
              <p className="text-display font-black leading-none text-content-primary">{googleReviewsMock.rating}</p>
              <StarRating rating={Math.round(googleReviewsMock.rating)} size={20} />
            ...
            {googleReviewsMock.isMock && (
              <div className="mx-auto mb-10 max-w-3xl rounded-md border border-warning/40 bg-warning-bg/40 px-5 py-3 text-center">
                <p className="text-caption leading-relaxed text-content-secondary">
                  <strong className="text-content-primary">Esta sección es una muestra.</strong>{" "}
                  Cuando Cristian vincule su Google Business Profile, las reseñas se cargarán
                  automáticamente desde Google Maps en tiempo real.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {googleReviewsMock.reviews.map((review, i) => (
                <GoogleReviewCard key={`${review.name}-${i}`} review={review} index={i} />
              ))}
            </div>
            ...
            <div className="mt-10 text-center">
              {isExternal ? (
                ...
                <p className="text-caption text-content-disabled">
                  Link al perfil disponible una vez vinculado el Google Business Profile.
                </p>
              )}
            </div>
          </div>
