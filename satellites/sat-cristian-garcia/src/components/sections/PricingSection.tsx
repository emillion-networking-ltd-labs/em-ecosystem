    ...
    >
      {plan.highlighted && (
        <div className="absolute -top-3 left-6">
          <Badge variant="warning" size="sm">Mas elegido</Badge>
        </div>
      )}
      <h3 className="text-h2 font-semibold text-content-primary">{plan.name}</h3>
      <div className="mt-2 flex items-baseline gap-1">
        <span className="text-3xl font-black text-content-primary">{plan.price}&euro;</span>
        <span className="text-caption text-content-tertiary">{plan.period}</span>
      </div>
      <p className="mt-2 text-body text-content-secondary">{plan.description}</p>
      ...
      <div className="mt-6">
        <Button variant={plan.highlighted ? "primary" : "outline"} size="lg" fullWidth href="/contacto">{plan.cta}</Button>
      </div>
    </div>
  );
}
    ...
    <div className="mx-auto max-w-7xl px-6">
      <div className="mb-12 text-center">
        <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
          <span className="text-[18px] leading-7 md:text-h1 font-semibold tracking-wide text-accent">Precios »»</span>
          <h2 className="text-[18px] leading-7 md:text-h1 font-bold text-content-primary">Invierte en ti</h2>
        </div>
        <p className="mx-auto mt-3 max-w-xl text-base text-content-secondary">Elige el plan que mejor se adapte a tus objetivos.</p>
      </div>
