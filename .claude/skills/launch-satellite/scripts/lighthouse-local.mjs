...
import { RUNS, hasChromeInPath, measureMedians, reportMedians, reportMetrics } from "./lib/lighthouse.mjs";
...
const passed = reportMedians(result);
reportMetrics(result);
if (!passed) {
  console.error("lighthouse-local: S2 local NO alcanzado (mediana) — reportar el gap (no falsear)."); process.exit(1);
}
console.log("lighthouse-local: S2 local PASS (mediana ≥ umbrales).");
