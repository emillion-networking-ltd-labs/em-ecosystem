// Gate de CAPTURA / COMPLETITUD — fuente → IR (FB0/G1, ECO-63/ECO-73 · ADR-012). El fallo histórico fue la
// PÉRDIDA SILENCIOSA: páginas/imágenes/bloques que se caían sin avisar. Este gate lo hace IMPOSIBLE de ignorar:
// compara lo que la fuente CONTENÍA (coverage declarada por el adapter) con lo que el IR CAPTURÓ (medido del IR
// real), y FALLA si se perdió algo. Honestidad (ADR-012): "nada se pierde" está acotado a lo que el backup
// contiene — lo que la fuente NO podía dar (p.ej. contenido dinámico ausente) se DECLARA como `notInSource`, no
// se cuenta como pérdida ni se inventa.
//
// El adapter adjunta `ir.coverage = { source:{pages,blocks,media,...}, captured:{...}, dropped:[{what,why}],
//   subfields:{ seo:{source:N} }, notInSource:[...] }`. Aquí se evalúa y se reporta. GENÉRICO: sólo conoce el IR
// (model/ir.mjs) — cero conocimiento de la fuente (eso vive en el adapter). El gate de EMISIÓN (IR → sitio) es
// su simétrico y vive aparte (lib/lossless.mjs hoy; standard/ en G2).
...
// Lanza si el gate de captura falla (uso desde el orquestador / CI del builder).
export function assertLossless(ir) {
  const r = losslessReport(ir);
  if (!r.ok) {
    const e = new Error("gate lossless de CAPTURA FALLÓ — el IR perdería información que la fuente tenía:\n" + r.problems.map((p) => "  - " + p).join("\n"));
    e.report = r;
    throw e;
  }
  return r;
