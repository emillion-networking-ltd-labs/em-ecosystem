...
export default function ChangePasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasPassword) {
        addToast({ variant: 'success', title: 'Password changed', description: 'Please log in again with your new password.' });
        await logout();
        router.push('/login');
        return;
      }
      addToast({ variant: 'success', title: 'Password set', description: 'Your password has been updated.' });
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
      ...
      addToast({ variant: 'error', title: 'Password change failed', description: msg.endsWith('.') ? msg : `${msg}.` });
    } finally {
      setLoading(false);
    }
  };
