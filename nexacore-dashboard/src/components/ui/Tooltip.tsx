    ...
    >
      ...
      {visible && (
        <div
          id={tooltipId}
          role="tooltip"
          className={`absolute z-50 max-w-[241px] rounded-xs border border-border-default bg-white px-4 py-3 shadow-card dark:bg-surface-card ${positionClasses[position]}`}
        >
          <p className="text-caption font-medium text-content-primary">
            {content}
          </p>
          <div
            className={`absolute h-[8px] w-[8px] rotate-45 border border-border-default bg-white dark:bg-surface-card ${arrowClasses[position]}`}
          />
        </div>
      )}
    </div>
