...
const DETECTORS = [
  ...
  // Lookbehind de LETRA (no `\b`, ECO-204/ex-ECO-201): `\b` no separa `_` de `rgba` (ambos son \w), así que el
  // negro crudo en un valor arbitrario `shadow-[0_1px_3px_rgba(0,0,0,0.25)]` (Tailwind codifica los espacios
  // con `_`) se COLABA. `(?<![a-zA-Z])` sí caza `_rgba(` / `,rgba(` y sigue exigiendo que no sea parte de palabra.
  ["rgb/hsl", /(?<![a-zA-Z])(?:rgba?|hsla?)\s*\(\s*(?!var)/],
...
function walk(dir) {
  ...
  for (const e of entries) {
    ...
    if (!e.isFile() || !CODE_EXT.test(e.name) || EXEMPT_BASENAME.has(e.name))
      continue;
  ...
  if (REPORT || hits.length) {
    ...
    for (const [file, hs] of [...byFile.entries()].sort(
      ...
      for (const h of hs.slice(0, REPORT ? 100 : 6))
        console.log(`    :${h.line} [${h.kind}] ${h.text}   — ${h.ctx}`);
    }
  }
  if (!hits.length) {
    console.log(
      "✓ check-raw-color OK — sin valores de color crudo fuera de los tokens.",
    );
  } else if (!REPORT) {
    console.error(
