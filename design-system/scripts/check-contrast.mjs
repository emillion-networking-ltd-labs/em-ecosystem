...
export const SURFACES = ["surface-primary", "surface-secondary", "surface-tertiary"];
export function buildPairs() {
  ...
  for (const s of SURFACES) {
    PAIRS.push({ fg: "accent", bg: s, kind: "text", min: 4.5, blocking: false });
  }
  return PAIRS;
}

// Corre la matriz sobre una paleta {light, dark}. Devuelve {rows, failed}. `label` prefija cada fila.
export function runContrast({ light, dark }, { label = "", pairs = buildPairs() } = {}) {
  ...
  for (const theme of ["light", "dark"]) {
    for (const p of pairs) {
      const fgv = val(theme, p.fg), bgv = val(theme, p.bg);
      ...
      rows.push(`  [${flag}] ${pfx}${theme.padEnd(5)} ${p.fg} on ${p.bg}: ${r.toFixed(2)}:1 (min ${p.min}${p.blocking ? "" : ", info"})`);
    }
  }
  return { rows, failed };
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const themes = parseThemes(readFileSync(join(ds, "tokens", "tokens.css"), "utf8"));
