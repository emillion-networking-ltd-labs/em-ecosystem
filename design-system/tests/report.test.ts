...
function makeConsumer(files: Record<string, Copy>): string {
  ...
  for (const [key, f] of Object.entries(files)) {
    ...
    manifest.files[key] = f.hold
      ? { from: f.from, sha: f.sha, hold: true }
      : { from: f.from, sha: f.sha };
  }
  writeFileSync(
    join(root, "src", "em-ui.manifest.json"),
...
describe("classifyEntry (clasificador de sync)", () => {
  ...
  it("up-to-date: fiel a la base y la base == DS actual", () => {
    expect(classifyEntry({ from: "x", sha: baseSha }, baseSha, base)).toBe(
      STATUS.UP_TO_DATE,
    );
  });

  it("stale (ámbar, no rojo): fiel a la base pero el DS avanzó", () => {
    const s = classifyEntry(
      { from: "x", sha: baseSha },
      blobSha(Buffer.from("NEWER DS\n")),
      base,
    );
  ...
  it("held (no rojo): congelado gana a stale/drift", () => {
    const s = classifyEntry(
      { from: "x", sha: baseSha, hold: true },
      blobSha(Buffer.from("NEW\n")),
      Buffer.from("EDIT\n"),
    );
  ...
  it("reporta TODOS los consumidores aunque el 1º esté ROJO (no para en el 1er rojo) y marca rojo el roto", () => {
    ...
    const b = makeConsumer({
      "components/ui/Badge.tsx": {
        ...
        sha: blobSha(badgeBuf),
        bytes: badgeBuf,
      },
      "components/ui/Button.tsx": {
        from: "components/Button.tsx",
        sha: blobSha(Buffer.from("base\n")),
        hold: true,
        bytes: "lo que sea\n",
      },
      "styles/em-ui-tokens.css": {
        from: "tokens/tokens.css",
        sha: blobSha(tokensBuf),
        bytes: tokensBuf,
      },
    });
