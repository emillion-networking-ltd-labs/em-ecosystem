...
1. **OnlineMlScorerInterceptor** [SCRUM-462; updated SCRUM-464] (from OnlineMlScorerModule) — Calls `OnlineMlScorerService.score()` post-handler in `tap.next`/`tap.error`. AUTH-skip via centralized constant `AUTH_SKIP_PATHS` (`src/common/constants/auth-skip-paths.constants.ts`, imported by both interceptor and service per defense-in-depth dupla; mirror of the auth-skip-paths framework template). Shadow-mode only (writes JSONL log; never blocks). Fail-open on errors.
