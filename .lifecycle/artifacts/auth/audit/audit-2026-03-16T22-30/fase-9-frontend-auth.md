...
- `app/profile/error.tsx` — Profile-specific error boundary: catches profile page crashes, shows "Try again" + dashboard link

**Finding**: Auth-specific route error boundaries are missing. There is no `app/login/error.tsx`, `app/register/error.tsx`, `app/forgot-password/error.tsx`, or `app/reset-password/error.tsx`. The root `app/error.tsx` will catch crashes in these routes, but it shows a generic full-page error without auth-specific context (e.g., "Go to login" link). The root boundary is adequate for production resilience but does not provide optimized auth-flow recovery.

**Mitigation**: The root `app/error.tsx` provides adequate catch-all protection, and the `app/global-error.tsx` handles even root layout failures. This is a best-practice gap, not a functional deficiency.
...
- **Keyboard navigation**: Forms use standard `<form>` with `<button type="submit">`. Password show/hide toggle has `aria-label`. MFA digit inputs have `aria-label="Digit ${i+1}"` (`MfaTotpStep.tsx:275`). Passkey buttons have `aria-label` (`PasskeyManager.tsx:71,79`).
...
**Finding**: The MFA TOTP 6-digit input group (`MfaTotpStep.tsx:256-278`) uses individual `<input>` elements with `aria-label="Digit N"` and a group `role="group" aria-label="Verification code digits"`. However, the group label "Verification Code" above (`MfaTotpStep.tsx:251`) is a standalone `<label>` not linked via `htmlFor` to any specific input (the group is a `<div>`, not an input). Screen readers may not associate the label with the digit group. This is a minor a11y gap; the `role="group"` + `aria-label` on the container provides reasonable context.
