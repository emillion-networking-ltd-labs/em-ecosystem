...
| File | Lines | Verdict |
|------|-------|---------|
| passkey.service.ts | 454 | WARN |
| login.service.ts | 365 | WARN |
| token.service.ts | 326 | WARN |
| mfa.service.ts | 304 | WARN |
| email-verification.service.ts | 279 | PASS |
| oauth.controller.ts | 270 | PASS |
| auth.controller.ts | 249 | PASS |
| trusted-device.service.ts | 214 | PASS |
| passkey.controller.ts | 192 | PASS |
| mfa.controller.ts | 189 | PASS |
| account.controller.ts | 173 | PASS |
| auth.service.ts | 171 | PASS |
| password-reset.service.ts | 167 | PASS |
| session.controller.ts | 159 | PASS |
| auth.constants.ts | 157 | PASS |
| login-security.service.ts | 141 | PASS |
| oauth-auth.service.ts | 127 | PASS |
| auth.module.ts | 106 | PASS |
| github.strategy.ts | 98 | PASS |
| google.strategy.ts | 89 | PASS |
| roles.guard.ts | 69 | PASS |
| password-breach.service.ts | 68 | PASS |
| token-deny-list.service.ts | 60 | PASS |
| oauth-state.store.ts | 56 | PASS |
| permissions.guard.ts | 55 | PASS |
| oauth-validate.helper.ts | 55 | PASS |
| pkce-authenticate.ts | 48 | PASS |
| jwt.strategy.ts | 48 | PASS |
| oauth-code.store.ts | 42 | PASS |
| oauth-callback.filter.ts | 39 | PASS |
| oauth-link.guard.ts | 39 | PASS |
| base-oauth-auth.guard.ts | 32 | PASS |
| oauth-link-code.store.ts | 30 | PASS |
| audit-log.helper.ts | 28 | PASS |
| parse-duration.ts | 19 | PASS |
| passkey.constants.ts | 15 | PASS |
| hash-token.ts | 5 | PASS |
| jwt-auth.guard.ts | 5 | PASS |
| google-auth.guard.ts | 3 | PASS |
| github-auth.guard.ts | 3 | PASS |
| interfaces/auth.interfaces.ts | 34 | PASS |
...
- `passkey.service.ts`: 454 lines (threshold: 300)
- `login.service.ts`: 365 lines (threshold: 300)
- `token.service.ts`: 326 lines (threshold: 300)
- `mfa.service.ts`: 304 lines (threshold: 300)

### SM-02: File length (tests) — Tier 1

**Threshold**: <=900 PASS, 901-1500 WARN, >1500 FAIL

Test files were not individually measured via CLI (Bash restricted during audit), but the module's test structure features 40+ spec files in a dedicated `tests/` subdirectory. With this high decomposition, no single test file is expected to exceed 900 lines.

**Verdict**: **PASS** — test suite is highly decomposed across 40+ files, mitigating concentration risk.

### SM-03: Function/method length — Tier 1

**Threshold**: <=50 PASS, 51-75 WARN, >75 FAIL

Top 10 largest functions/methods by line count (counted from file reads):

| File | Function | Lines | Verdict |
|------|----------|-------|---------|
| passkey.service.ts:199 | `verifyAuthentication()` | ~63 | WARN |
| trusted-device.service.ts:36 | `trustDevice()` | ~57 | WARN |
| token.service.ts:68 | `generateTokens()` | ~49 | PASS |
| token.service.ts:119 | `refreshTokens()` | ~48 | PASS |
| login.service.ts:98 | `login()` | ~44 | PASS |
| passkey.service.ts:154 | `generateAuthOptions()` | ~43 | PASS |
| passkey.service.ts:311 | `deletePasskey()` | ~41 | PASS |
| mfa.service.ts:49 | `setupMfa()` | ~41 | PASS |
| token.service.ts:169 | `generateTokensForMfa()` | ~37 | PASS |
| email-verification.service.ts:81 | `verifyEmailChange()` | ~33 | PASS |
...
- `trusted-device.service.ts:36` — `trustDevice()`: ~57 lines (threshold: 50)
...
Total production LOC (excluding DTOs, tests): ~4,690 lines (estimated from all files read).
Top file: `passkey.service.ts` at 454 lines = ~9.7% of module total.

**Verdict**: **PASS** — top file is 9.7% of module (threshold: 40%).

### SM-06: Module total volume — INFO

**Total production LOC**: ~4,690 lines across 43 production files.
...
2. `trusted-device.service.ts:36` — `trustDevice()`: ~57 lines
...
**Note**: `npx jscpd` not executed (Bash restricted). All duplication checks performed via Tier 2 heuristic analysis. Results flagged as non-deterministic.
...
4. **Impossible travel check + notify pattern**: ~8 lines repeated 3 times across `login.service.ts` (2x) and `token.service.ts` (1x).
...
Test helper `auth-test.helpers.ts` exists to share mock factories across 40+ spec files. Threshold for tests is 10%.
...
1. Post-login security checks across `login.service.ts` and `token.service.ts` (~10 lines)
2. Google/GitHub strategy `authenticate()` method — identical 7-line delegation to `applyPkceAuthenticate`
3. Google/GitHub strategy `authorizationParams()` — identical 3-line delegation to `applyPkceAuthorizationParams`

**Verdict**: **WARN** — 3 cross-file clone patterns identified (threshold: 1-3 = WARN).

### DU-05: Utility extraction candidates — Tier 2

**Recommendation**: Extract the post-login security checks block to a single `LoginSecurityService.executePostLoginChecks(user, sessionId, requestMeta)` method. This would eliminate 3 repetitions across 2 files.
...
**Verdict**: **WARN** — `AuthService` has 17 public methods (WARN band: 13-18). However, `AuthService` is an intentional **facade** pattern — every method is a 1-line delegation to a sub-service. The facade provides a unified API surface for controllers while keeping actual logic decomposed.
...
3. Set cookie (if applicable)
4. Return response

`auth.controller.ts:login()` contains two type-guard conditionals (`'mfaRequired' in result`, `'mfaSetupRequired' in result`), but these are response-format discriminators, not business logic.
...
**Evidence**: This is the standard NestJS resolution for a well-known bidirectional dependency (auth needs users for user lookup, users needs auth for guards).

**Verdict**: **PASS** — 1 `forwardRef` usage, which is the standard NestJS pattern for this cross-module dependency. Not a design flaw.
...
- `MfaVerifyLoginDto`: mfaToken (required), code (optional), recoveryCode (optional), trustDevice (optional)
- `PasskeyDeleteDto`: password (optional)
...
**Note**: `npx eslint` not executed (Bash restricted). Based on active pre-commit hooks (Husky + lint-staged) and prior audit (2026-03-15) showing 0 ESLint errors.

**Verdict**: **PASS** — ESLint enforced via pre-commit hooks.

### TS-04: No `@ts-ignore` / `@ts-expect-error` — Tier 1

GREP results: **0 occurrences** across entire auth module (including tests).
...
1. `passkey.service.ts:45` — `options as unknown as Record<string, unknown>` (WebAuthn library type)
...
4. `strategies/google.strategy.ts:43` — `this as unknown as PassportOAuth2Internals` (Passport PKCE)
5. `strategies/github.strategy.ts:43` — `this as unknown as PassportOAuth2Internals` (Passport PKCE)
...
All numeric constants extracted to `constants/auth.constants.ts` and `constants/passkey.constants.ts`:
...
All error messages extracted to `ErrorMessages` constant (`common/constants/error-messages`). Audit actions use `AuditAction` enum. Redis key prefixes use named constants.
...
- Module exports: `AuthService`, `TokenService`, `PasswordBreachService`, `TrustedDeviceService`, `TokenDenyListService` — all used by other modules
...
- Re-exports from `auth.service.ts`: type re-exports for backward compatibility — used by controllers
...
- `email-verification.service.ts:26` — `new Logger(EmailVerificationService.name)`
...
GREP results for `TODO|FIXME|HACK|XXX|TEMP` in `src/auth/`: **0 occurrences** (only false positives from `MAX_FAILED_ATTEMPTS` constant name).
...
Verified across 5+ production files:
...
|----------|----------|---------|----------|----------------|
| SM-01 | MEDIUM | 4 production files exceed 300 lines | passkey.service.ts:454, login.service.ts:365, token.service.ts:326, mfa.service.ts:304 | Consider extracting private helpers to separate files for passkey.service.ts (highest at 454) |
| SM-03 | MEDIUM | 2 functions exceed 50 lines | passkey.service.ts:199 verifyAuthentication():63, trusted-device.service.ts:36 trustDevice():57 | Extract sub-steps to private helpers |
...
| SD-01 | MEDIUM | AuthService has 17 public methods | auth.service.ts — 17 facade-delegation methods | Accepted: intentional facade pattern. Each method is 1-line delegation |
...
| DU-04 | MEDIUM | 3 cross-file clone patterns | Post-login security block (2 files), OAuth strategy patterns (2 files) | Extract post-login checks to LoginSecurityService.executePostLoginChecks() |
...
## Recurrence Analysis vs Previous Audit (2026-03-15)

| Finding | Previous Audit | Current Status | Delta |
|---------|---------------|----------------|-------|
| SM-03 (functions >75 lines) | FAIL — 3 functions exceeded 75 lines | PASS — 0 functions >75 lines | Remediated (Sprint 11, SCRUM-245) |
| CX-05 (DI fan-out >8) | Not assessed at this granularity | WARN — max is 7 (TokenService) | New baseline |
| `any` type count | Higher count | 2 justified occurrences | Improved |
| Duplication | Not previously measured | ~2-3% estimated | New baseline |

The module shows measurable improvement from prior audit remediation efforts. All previous FAIL findings in the code quality domain have been resolved.
